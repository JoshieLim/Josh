...
def sort_detection(textboxes, image):
    height = image.shape[0]
    y_gap = 50
    detections = textboxes.copy()
    container = []
    for pixel in range(y_gap, height, y_gap):
        temp_detections = []
        ...
        pred_len = len(temp_detections)
        iteration = pred_len - 1
        for i in range(0, pred_len):
            for j in range(0, iteration-i):
                x1 = min(temp_detections[j][:,0])
                x2 = max(temp_detections[j][:,0])
                mid_x = ((x1+ x2)/2)

                next_x1 = min(temp_detections[j+1][:,0])
                next_x2 = max(temp_detections[j+1][:,0])
                next_mid_x = ((next_x1+next_x2) / 2 )
                if mid_x > next_mid_x:
                    temp = temp_detections[j]
                    temp_detections[j] = temp_detections[j+1]
                    temp_detections[j+1] = temp
            
        container = container + temp_detections
    return container
def get_xy_minmax(detection):
    minmax ={
            'x1':max(int(min(detection[:,0])), 0),
            'y1':max(int(min(detection[:,1])), 0),
            'x2':int(max(detection[:,0])),
            'y2':int(max(detection[:,1]))
        }
    return minmax
def get_textbox(text, predictions):
    for prediction in predictions:
        ...
        if text.upper() == pred_text.upper():
            return {
                'x1':max(int(min(prediction[1][:,0])), 0),
                'y1':max(int(min(prediction[1][:,1])), 0),
                'x2':int(max(prediction[1][:,0])),
                'y2':int(max(prediction[1][:,1]))
            }
    return 'Not found'
...
def filter_corrections(corrections, color):
